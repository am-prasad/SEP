...
// Get backend base URL (e.g., http://localhost:5000 or http://10.x.x.x)
const BASE = (import.meta.env.VITE_API_BASE_URL || '').replace(/\/$/, '');

const ItemCard = ({ item }) => {
  // Final image URL: use full backend path if imageUrl exists
  const imageSrc = item.imageUrl ? `${BASE}/uploads/${item.imageUrl}` : null;
